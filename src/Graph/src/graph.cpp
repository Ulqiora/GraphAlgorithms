...
namespace s21 {
    ...
    void Graph::initTypes() {
        for (int i = 0; i < adjacencyMatrix.size(); ++i) {
            for (int j = 0; j < adjacencyMatrix.size(); ++j) {
                ...
                if (i == j && std::fabs(adjacencyMatrix(i, j)) >
                                      std::numeric_limits<double>::epsilon())
                    loop = true;
            }
        }
    ...
    void Graph::printInfoAboutEdge(int firstNode, int secondNode,
                                   std::ofstream& file,
                                   const std::string& EdgesInFile,
                                   const Matrix& temp) {
        ...
        std::string weight = (typeWeights == TypeGraphByWeights::WEIGHTED)
                                     ...
                                     : "";
        if (std ::fabs(temp(firstNode - 1, secondNode - 1)) >
            std::numeric_limits<double>::epsilon()) {
            if (temp(firstNode - 1, secondNode - 1) >
                std::numeric_limits<double>::epsilon()) {
                file << "\t" << first << " " << EdgesInFile << " " << second << " ["
                     << std::endl;
            } else {
                file << "\t" << second << " " << EdgesInFile << " " << first << " ["
                     << std::endl;
            }
