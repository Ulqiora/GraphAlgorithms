...
namespace s21 {
    ...
    int generateRandomValue(int minimum, int maximum) {
        ...
        std::mt19937 rng(dev());
        std::uniform_int_distribution<std::mt19937::result_type> dist(minimum,
                                                                      maximum);
        return dist(rng);
    }
    ...
    void GraphAlgorithms::addToResultForDepth(stack<int> &stackForAlgorithm,
    ...
    void GraphAlgorithms::addToResultForBreadth(queue<int> &queueForAlgorithm,
                                                std::vector<int> &result) {
        if (!queueForAlgorithm.empty()) result.push_back(queueForAlgorithm.top() + 1);
    }

    TsmResult GraphAlgorithms::solveTravelingSalesmanProblem(Graph &graph) {
        if (!graph.isCompliteGraph())
            throw std::invalid_argument("This graph is not complete!");
        if (graph.hasNegativeWeights())
            throw std::invalid_argument("This graph has negative weights!");
        if (graph.size() < 2)
            throw std::invalid_argument("This graph is very small! Try again!");
        AntAlgorithm aa;
        ...
        while (visitedVertices.size() < size) {
            ...
            for (auto i: visitedVertices)
                for (size_t j = 0; j < size; j++)
                    if (isShortestPath(matrix(j, i), minWeight) &&
                        !visitedVertices.count(j)) {
    ...
    double GraphAlgorithms::getShortestPathBetweenVertices(Graph &graph,
        ...
        std::vector<double> minRoad(graph.size(),
    ...
    TsmResult GraphAlgorithms::solveTravelingSalesmanProblemBrudeForce(
            Graph &graph) {
        if (!graph.isCompliteGraph())
            throw std::invalid_argument("This graph is not complete!");
        if (graph.hasNegativeWeights())
