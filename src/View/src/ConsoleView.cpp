...
void ConsoleView::loadGraphMethod() {
  ...
  try {
    ...
    std::cout << "Graph loaded succesfully. Graph size = " << myGraph.size()
              << std::endl;
  } catch (std::exception& e) {
    std::cout << e.what() << std::endl;
  }
    ...
    if (isNumber(firstVertex) && isNumber(secondVertex)) {
      try {
        ...
        if (firstNum != secondNum && firstNum > 0 && secondNum > 0 &&
          ...
          std::cout << myAlgorithms.getShortestPathBetweenVertices(
                           myGraph, firstNum, secondNum)
                    << std::endl;
        } else {
...
void ConsoleView::salesManCompareMethod() {
  if (myGraph.size() > 0) {
    ...
    if (isNumber(strCycles)) {
      ...
      if (numCycles > 0) {
        printMessage(bonus);
        std::cout << "Ant time, ms = " << timeCompare(numCycles, antAlgo)
                  << std::endl;
        std::cout << "BnB time, ms = " << timeCompare(numCycles, bnbAlgo)
                  << std::endl;
        std::cout << "BruteForce time, ms = "
                  << timeCompare(numCycles, bruteAlgo) << std::endl;
      } else {
...
double ConsoleView::timeCompare(int numCycles, const commandList& algo) {
  ...
  std::chrono::time_point<std::chrono::system_clock> tFinish =
      std::chrono::system_clock::now();
