...
class Ant {
 ...
 private:
  ...
  std::shared_ptr<std::map<int, double>> calcProbabilities(
      Graph* graph, const Matsrix& pheromonesMap) {
    std::map<int, double>* probabilities = new std::map<int, double>;
    double probability = 0.0;
    for (int i = 1; i < graph->size(); ++i) {
      if ((*graph)(path.back(), i) != 0 && !(passedСity[i])) {
        probability += calculateProbability(
            (*graph)(path.back(), i), pheromonesMap(path.back() - 1, i - 1));
  ...
  void chooseWay(Graph* graph,
                 std::shared_ptr<std::map<int, double>> probabilities) {
 ...
 public:
  ...
  void findPath(Graph* graph, const Matrix& pheromonesMap) {
    ...
    for (int i = 1; i < graph->size(); ++i) {
      std::shared_ptr<std::map<int, double>> probabilities =
          calcProbabilities(graph, pheromonesMap);
